...
const PendingSCB: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [statementsRes, depositsRes] = await Promise.all([
        adminTransferAPI.getSCBStatements({ limit: pageSize, offset: (page - 1) * pageSize }),
        adminTransferAPI.getPendingDepositsForSCB({ limit: 50, offset: 0 }),
      ])
  ...
  const handleMatch = async () => {
    ...
    try {
      await adminTransferAPI.matchSCBDeposit({
  ...
  const handleUnmatch = async (statementId: string) => {
    ...
    try {
      await adminTransferAPI.unmatchSCBDeposit(statementId)
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-display font-bold text-gold-500">จับคู่รายการฝาก SCB</h1>
      ...
      <div className="bg-admin-card border border-admin-border rounded-xl shadow-lg overflow-hidden">
        <div className="p-6 border-b border-admin-border">
          <h2 className="text-xl font-display font-bold text-gold-500">รายการโอนเงินจากธนาคาร SCB</h2>
        </div>
